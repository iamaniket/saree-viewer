...
class GLTFLoader extends Loader {
  ...
  load(url, design1, blouseColor1, onLoad, onProgress, onError) {
    const scope = this;

    blouseColor = blouseColor1;
    design = design1;

    console.log("design: " + design);
    console.log("blouseColor: " + blouseColor);
...
class GLTFTextureBasisUExtension {
  ...
  loadTexture(textureIndex, designName, blouseColor) {
    const parser = this.parser;
    const json = parser.json;

    const textureDef = json.textures[textureIndex];

    console.log(designName + blouseColor);

    if (blouseColor !== "") {
      const index = json.images.findIndex((image) => image.name === "BLOUSE");
      if (index > -1) {
        json.images[index].uri = blouseColor;
      }
    }

    if (designName !== "") {
      const index = json.images.findIndex((image) => image.name === "SAREE");
      if (index > -1) {
        json.images[index].uri = designName;
      }
    }
...
class GLTFTextureWebPExtension {
  ...
  loadTexture(textureIndex, designName, blouseColor) {
    ...
    return this.detectSupport().then(function (isSupported) {
      ...
      return parser.loadTexture(textureIndex, designName, blouseColor);
    });
  }
    ...
    if (!dependency) {
      switch (type) {
        ...
        case "texture":
          dependency = this._invokeOne(function (ext) {
            return (
              ext.loadTexture && ext.loadTexture(index, design, blouseColor)
            );
          });
          break;
